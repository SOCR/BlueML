...
module.exports = function(app) {
    ...
    app.get('/rest/results', async function(req, res) {
        ...
        for (let i = 1; i < 50; i++) {
            let test_patient = data_tf[i].splice(1);
            test_patient = test_patient.slice(0, -1);  // Remove y column
            let sum = 0;
            for (let j =  0; j < test_patient.length; j++) {
                sum += test_patient[j];
            }
            console.log(test_patient[test_patient.length - 1]);
            console.log(sum / test_patient.length);
            const patient_id = 0;
